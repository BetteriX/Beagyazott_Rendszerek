...
def draw_boxes(image, boxes, ration):
    ...
    for startX, startY, endX, endY in boxes:
        ...
        pad = 5
        roi = image[
            max(0, startY - pad) : min(image.shape[0], endY + pad),
            max(0, startX - pad) : min(image.shape[1], endX + pad),
        ]
        ...
        # Store the whole text located in the picture
    ...
    for (startX, startY, endX, endY), text in results:
        ...
        cv2.putText(
            image, text, (startX, y), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2
        )
